...
static void scene_next_task_cb(lv_task_t * task)
{
    ...
    if(scenes[scene_act].create_cb) {
        ...
        if(opa_mode) {
            lv_label_set_text_fmt(subtitle, "\"%s\": %d FPS", scenes[scene_act].name, scenes[scene_act].fps_normal);
        } else {
            if(scene_act > 0) {
                lv_label_set_text_fmt(subtitle, "\"%s + opa\": %d FPS", scenes[scene_act - 1].name, scenes[scene_act - 1].fps_opa);
            } else {
